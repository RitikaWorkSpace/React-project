// import React, { useState, useEffect, useRef } from "react";
// import { Link } from "react-router-dom";
// import { useDispatch, useSelector } from "react-redux";
// import { useNavigate } from "react-router-dom";
// import Slider from "react-slick";
// // custom
// import { fetchProduct } from "../Redux/slices/productSlice";
// import { addToCart } from "../Redux/slices/cartSlice";
// import Rating from "./Rating";
// import { toggleLike } from "../Redux/slices/likeSlice";

// const Card = ({CustomArrowLeft,CustomArrowRight}) => {
//   // product show
//   const dispatch = useDispatch();
//   const navigate = useNavigate();
//   const likedItems = useSelector((state) => state.likes.likedItems);
//   const product = useSelector((state) => state.products.product);

//   useEffect(() => {
//     dispatch(fetchProduct());
//   }, []);
//   const status = useSelector((state) => state.products.status);
//   if (!product) {
//     return <div>Loading...</div>;
//   }
//   // product show

//   // add to cart with reducer
//   const handleAddToCart = (item) => {
//     dispatch(addToCart(item));
//     navigate("/CartPage");
//   };


//   const settings = {
//     infinite: true,
//     speed: 500,
//     slidesToShow: 4,
//     slidesToScroll: 1,
//     nextArrow: CustomArrowRight ? <CustomArrowRight /> : false,
//     prevArrow: CustomArrowLeft ? <CustomArrowLeft /> : false,
//   };

// const view = useNavigate();
//   const handleView = (id)=>{
//  view(`/Vp/${id}`);
//   }


//   return (
  


     
//       <div className="flex gap-[30px] ">
//         <Slider {...settings}>

      
//         {product?.map((item, i) => (
//           <div className="max-w-[270px] w-full max-h-[350px] h-full  " key={i}>
//             {/* Card Container */}
//             <div className="bg-white rounded-lg shadow-md overflow-hidden">
//               {/* Card Image */}
//               <div className="relative w-[270px] h-[250px] bg-[#F5F5F5] group">
//                 <div className="absolute top-2 left-2 bg-[#DB4444] text-white p-1 rounded-md">
//                   -40%
//                 </div>
//                 <div className="absolute top-2 right-2 flex flex-col gap-2">
//                   <button  className={`p-2 rounded-full ${
//                     likedItems[item.id] ? 'bg-[#DB4444]' : 'bg-white'
//                   }`} onClick={() => dispatch(toggleLike(item.id))}>
//                     <svg
//                       className="h-6 w-6 text-gray-600"
//                       xmlns="http://www.w3.org/2000/svg"
//                       viewBox="0 0 512 512"
//                     >
//                       {/* Heart Icon */}
//                       <path d="M225.8 468.2l-2.5-2.3L48.1 303.2C17.4 274.7 0 234.7 0 192.8l0-3.3c0-70.4 50-130.8 119.2-144C158.6 37.9 198.9 47 231 69.6c9 6.4 17.4 13.8 25 22.3c4.2-4.8 8.7-9.2 13.5-13.3c3.7-3.2 7.5-6.2 11.5-9c0 0 0 0 0 0C313.1 47 353.4 37.9 392.8 45.4C462 58.6 512 119.1 512 189.5l0 3.3c0 41.9-17.4 81.9-48.1 110.4L288.7 465.9l-2.5 2.3c-8.2 7.6-19 11.9-30.2 11.9s-22-4.2-30.2-11.9zM239.1 145c-.4-.3-.7-.7-1-1.1l-17.8-20-.1-.1s0 0 0 0c-23.1-25.9-58-37.7-92-31.2C81.6 101.5 48 142.1 48 189.5l0 3.3c0 28.5 11.9 55.8 32.8 75.2L256 430.7 431.2 268c20.9-19.4 32.8-46.7 32.8-75.2l0-3.3c0-47.3-33.6-88-80.1-96.9c-34-6.5-69 5.4-92 31.2c0 0 0 0-.1 .1s0 0-.1 .1l-17.8 20c-.3 .4-.7 .7-1 1.1c-4.5 4.5-10.6 7-16.9 7s-12.4-2.5-16.9-7z" />
//                     </svg>
//                   </button>
//                   {/* view icon */}
            
//                     <button className="p-2 bg-white rounded-full" onClick={()=>handleView(item.id)}>
//                       <svg
//                         className="h-6 w-6 text-gray-600"
//                         xmlns="http://www.w3.org/2000/svg"
//                         viewBox="0 0 576 512"
//                       >
//                         {/* View Icon */}
//                         <path d="M288 80c-65.2 0-118.8 29.6-159.9 67.7C89.6 183.5 63 226 49.4 256c13.6 30 40.2 72.5 78.6 108.3C169.2 402.4 222.8 432 288 432s118.8-29.6 159.9-67.7C486.4 328.5 513 286 526.6 256c-13.6-30-40.2-72.5-78.6-108.3C406.8 109.6 353.2 80 288 80zM95.4 112.6C142.5 68.8 207.2 32 288 32s145.5 36.8 192.6 80.6c46.8 43.5 78.1 95.4 93 131.1c3.3 7.9 3.3 16.7 0 24.6c-14.9 35.7-46.2 87.7-93 131.1C433.5 443.2 368.8 480 288 480s-145.5-36.8-192.6-80.6C48.6 356 17.3 304 2.5 268.3c-3.3-7.9-3.3-16.7 0-24.6C17.3 208 48.6 156 95.4 112.6zM288 336c44.2 0 80-35.8 80-80s-35.8-80-80-80c-.7 0-1.3 0-2 0c1.3 5.1 2 10.5 2 16c0 35.3-28.7 64-64 64c-5.5 0-10.9-.7-16-2c0 .7 0 1.3 0 2c0 44.2 35.8 80 80 80zm0-208a128 128 0 1 1 0 256 128 128 0 1 1 0-256z" />
//                       </svg>
//                     </button>
                
//                 </div>
//                 <div className="flex justify-center items-center w-full h-full group ">
//                   <img
//                     src={item.image}
//                     alt="Product"
//                     className="w-[190px] h-[180px] object-contain "
//                   />
//                 </div>
//                 {/*add to card  */}
//                 <button
//                   className="text-center w-[100%] text-white bg-black p-[5px] absolute bottom-0 opacity-0 group-hover:!opacity-100 transition-opacity duration-300 cursor-pointer"
//                   onClick={() => handleAddToCart(item)}
//                 >
//                   Add to Cart
//                 </button>
//                 {/* add to card */}
//               </div>

//               {/* Card Body */}
//               <div className=" h-[100px] bg-white mt-[16px]">
//                 <div className="title font-poppins font-[500] text-[16px] leading-[24px] truncate max-w-[240px]">
//                   {item.title}
//                 </div>
//                 <div className="price text-[#DB4444] mt-2">
//                   ${item.price}
//                   <span className="text-[#aaaba7] ml-[10px] line-through">
//                     $160
//                   </span>
//                 </div>
//                 <Rating rate={item.rating.rate} count={item.rating.count} />
//               </div>
//               {/* Card Body */}
//             </div>
//           </div>
//         ))}
//           </Slider>
//       </div>

   
  
   
//   );
// };

// export default Card;






// import React, { useEffect } from "react";
// import { useDispatch, useSelector } from "react-redux";
// import { useNavigate } from "react-router-dom";
// import Slider from "react-slick";
// import { fetchProduct } from "../Redux/slices/productSlice";
// import { addToCart } from "../Redux/slices/cartSlice";
// import Rating from "./Rating";
// import { toggleLike } from "../Redux/slices/likeSlice";

// const Card = ({ CustomArrowLeft, CustomArrowRight }) => {
//   const dispatch = useDispatch();
//   const navigate = useNavigate();
//   const likedItems = useSelector((state) => state.likes.likedItems);
//   const product = useSelector((state) => state.products.product);



//   useEffect(() => {
//     dispatch(fetchProduct());
//   }, [dispatch]);

//   const handleAddToCart = (item) => {
//     dispatch(addToCart(item));
//     navigate("/CartPage");
//   };

//   const handleView = (id) => {
//     navigate(`/Vp/${id}`);
//   };

//   const settings = {
//     infinite: true,
//     speed: 500,
//     slidesToShow: 4,
//     slidesToScroll: 1,
//     nextArrow: CustomArrowRight ? <CustomArrowRight /> : null,
//     prevArrow: CustomArrowLeft ? <CustomArrowLeft /> : null,
//     responsive: [
//       {
//         breakpoint: 1024, 
//         settings: {
//           slidesToShow: 3,
//         },
//       },
//       {
//         breakpoint: 768, 
//         settings: {
//           slidesToShow: 2, 
//         },
//       },
//       {
//         breakpoint: 480, 
//         settings: {
//           slidesToShow: 1,
//         },
//       },
//     ],
//   };



//   return (
//     <div className="relative ">


//     <Slider {...settings}>
//   {product?.map((item) => (
//     <div className="px-3" key={item.id}>
//       <div className="bg-white rounded-lg shadow-md overflow-hidden max-w-[270px] mx-auto">
//         <div className="relative w-full h-[250px] bg-[#F5F5F5] group">
//           <div className="absolute top-2 left-2 bg-[#DB4444] text-white p-1 rounded-md">
//             -40%
//           </div>
//           <div className="absolute top-2 right-2 flex flex-col gap-2">
//             <button
//               className={`p-2 rounded-full ${likedItems[item.id] ? 'bg-[#DB4444]' : 'bg-white'}`}
//               onClick={() => dispatch(toggleLike(item.id))}
//             >
//               <svg
//                 className="h-6 w-6 text-gray-600"
//                 xmlns="http://www.w3.org/2000/svg"
//                 viewBox="0 0 512 512"
//               >
//                  <path d="M225.8 468.2l-2.5-2.3L48.1 303.2C17.4 274.7 0 234.7 0 192.8l0-3.3c0-70.4 50-130.8 119.2-144C158.6 37.9 198.9 47 231 69.6c9 6.4 17.4 13.8 25 22.3c4.2-4.8 8.7-9.2 13.5-13.3c3.7-3.2 7.5-6.2 11.5-9c0 0 0 0 0 0C313.1 47 353.4 37.9 392.8 45.4C462 58.6 512 119.1 512 189.5l0 3.3c0 41.9-17.4 81.9-48.1 110.4L288.7 465.9l-2.5 2.3c-8.2 7.6-19 11.9-30.2 11.9s-22-4.2-30.2-11.9zM239.1 145c-.4-.3-.7-.7-1-1.1l-17.8-20-.1-.1s0 0 0 0c-23.1-25.9-58-37.7-92-31.2C81.6 101.5 48 142.1 48 189.5l0 3.3c0 28.5 11.9 55.8 32.8 75.2L256 430.7 431.2 268c20.9-19.4 32.8-46.7 32.8-75.2l0-3.3c0-47.3-33.6-88-80.1-96.9c-34-6.5-69 5.4-92 31.2c0 0 0 0-.1 .1s0 0-.1 .1l-17.8 20c-.3 .4-.7 .7-1 1.1c-4.5 4.5-10.6 7-16.9 7s-12.4-2.5-16.9-7z" />
//               </svg>
//             </button>
//             <button className="p-2 bg-white rounded-full" onClick={() => handleView(item.id)}>
//               <svg
//                 className="h-6 w-6 text-gray-600"
//                 xmlns="http://www.w3.org/2000/svg"
//                 viewBox="0 0 576 512"
//               >
//                      <path d="M288 80c-65.2 0-118.8 29.6-159.9 67.7C89.6 183.5 63 226 49.4 256c13.6 30 40.2 72.5 78.6 108.3C169.2 402.4 222.8 432 288 432s118.8-29.6 159.9-67.7C486.4 328.5 513 286 526.6 256c-13.6-30-40.2-72.5-78.6-108.3C406.8 109.6 353.2 80 288 80zM95.4 112.6C142.5 68.8 207.2 32 288 32s145.5 36.8 192.6 80.6c46.8 43.5 78.1 95.4 93 131.1c3.3 7.9 3.3 16.7 0 24.6c-14.9 35.7-46.2 87.7-93 131.1C433.5 443.2 368.8 480 288 480s-145.5-36.8-192.6-80.6C48.6 356 17.3 304 2.5 268.3c-3.3-7.9-3.3-16.7 0-24.6C17.3 208 48.6 156 95.4 112.6zM288 336c44.2 0 80-35.8 80-80s-35.8-80-80-80c-.7 0-1.3 0-2 0c1.3 5.1 2 10.5 2 16c0 35.3-28.7 64-64 64c-5.5 0-10.9-.7-16-2c0 .7 0 1.3 0 2c0 44.2 35.8 80 80 80zm0-208a128 128 0 1 1 0 256 128 128 0 1 1 0-256z" />
//               </svg>
//             </button>
//           </div>
//           <div className="flex justify-center items-center w-full h-full group">
//             <img
//               className="w-[190px] h-[180px] object-contain"
//               src={item.image}
//               alt={item.title}
//             />
//           </div>
//           <button
//             className="text-center w-full text-white bg-black p-[5px] absolute bottom-0 opacity-0 group-hover:!opacity-100 transition-opacity duration-300 cursor-pointer"
//             onClick={() => handleAddToCart(item)}
//           >
//             Add to Cart
//           </button>
//         </div>

//         <div className="p-4">
//           <h2 className="title font-poppins font-[500] text-[16px] leading-[24px] truncate max-w-[240px]">{item.title}</h2>
//           <div className="price text-[#DB4444] mt-2">
//             ${item.price}
//             <span className="text-[#aaaba7] ml-[10px] line-through">
//               $160
//             </span>
//           </div>
//           <Rating rate={item.rating.rate} count={item.rating.count} />
//         </div>
//       </div>
//     </div>
//   ))}
// </Slider>

//     </div>
//   );
// };

// export default Card;



import React, { forwardRef, useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";
import { useNavigate } from "react-router-dom";
import  {GET_PRODUCT_DATA}  from '../NewRedux/actionTypes';
import { addToCart } from '../NewRedux/actions';

import Rating from "./Rating";
// import { toggleLike } from "../Redux/slices/likeSlice";

const Card = forwardRef((props,ref) => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  // const likedItems = useSelector((state) => state.likes.likedItems);
  const { isLoading, productData, error } = useSelector(state => state.product);


  useEffect(() => {
    
    dispatch({ type: GET_PRODUCT_DATA }); // Dispatching the action to trigger the saga
  }, [dispatch]);
  console.log(productData);

  const handleAddToCart = (item) => {
    dispatch(addToCart(item));
    navigate("/CartPage");
  };

  const handleView = (id) => {
    navigate(`/Vp/${id}`);
  };



  return (
    <div className=" relative   max-w-[1308px]  Slider flex  gap-[30px]  overflow-x-auto scroll-smooth' " ref={ref}
    style={{
      scrollbarWidth: 'none', 
      msOverflowStyle: 'none', 
    }}>
     
     {productData?.map((item, i) => (
          <div className="max-w-[270px] w-full  h-full  " key={i}>
            {/* Card Container */}
            <div className="bg-white rounded-lg shadow-md overflow-hidden">
              {/* Card Image */}
              <div className="relative w-[270px] h-[250px] bg-[#F5F5F5] group">
                <div className="absolute top-2 left-2 bg-[#DB4444] text-white p-1 rounded-md">
                  -40%
                </div>
                <div className="absolute top-2 right-2 flex flex-col gap-2">
                  {/* <button  className={`p-2 rounded-full ${
                    likedItems[item.id] ? 'bg-[#DB4444]' : 'bg-white'
                  }`} onClick={() => dispatch(toggleLike(item.id))}>
                    <svg
                      className="h-6 w-6 text-gray-600"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 512 512"
                    >
                      {/* Heart Icon */}
                      {/* <path d="M225.8 468.2l-2.5-2.3L48.1 303.2C17.4 274.7 0 234.7 0 192.8l0-3.3c0-70.4 50-130.8 119.2-144C158.6 37.9 198.9 47 231 69.6c9 6.4 17.4 13.8 25 22.3c4.2-4.8 8.7-9.2 13.5-13.3c3.7-3.2 7.5-6.2 11.5-9c0 0 0 0 0 0C313.1 47 353.4 37.9 392.8 45.4C462 58.6 512 119.1 512 189.5l0 3.3c0 41.9-17.4 81.9-48.1 110.4L288.7 465.9l-2.5 2.3c-8.2 7.6-19 11.9-30.2 11.9s-22-4.2-30.2-11.9zM239.1 145c-.4-.3-.7-.7-1-1.1l-17.8-20-.1-.1s0 0 0 0c-23.1-25.9-58-37.7-92-31.2C81.6 101.5 48 142.1 48 189.5l0 3.3c0 28.5 11.9 55.8 32.8 75.2L256 430.7 431.2 268c20.9-19.4 32.8-46.7 32.8-75.2l0-3.3c0-47.3-33.6-88-80.1-96.9c-34-6.5-69 5.4-92 31.2c0 0 0 0-.1 .1s0 0-.1 .1l-17.8 20c-.3 .4-.7 .7-1 1.1c-4.5 4.5-10.6 7-16.9 7s-12.4-2.5-16.9-7z" />
                    </svg>
                  </button> */} 
                  {/* view icon */}
            
                    <button className="p-2 bg-white rounded-full" onClick={()=>handleView(item.id)}>
                      <svg
                        className="h-6 w-6 text-gray-600"
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 576 512"
                      >
                        {/* View Icon */}
                        <path d="M288 80c-65.2 0-118.8 29.6-159.9 67.7C89.6 183.5 63 226 49.4 256c13.6 30 40.2 72.5 78.6 108.3C169.2 402.4 222.8 432 288 432s118.8-29.6 159.9-67.7C486.4 328.5 513 286 526.6 256c-13.6-30-40.2-72.5-78.6-108.3C406.8 109.6 353.2 80 288 80zM95.4 112.6C142.5 68.8 207.2 32 288 32s145.5 36.8 192.6 80.6c46.8 43.5 78.1 95.4 93 131.1c3.3 7.9 3.3 16.7 0 24.6c-14.9 35.7-46.2 87.7-93 131.1C433.5 443.2 368.8 480 288 480s-145.5-36.8-192.6-80.6C48.6 356 17.3 304 2.5 268.3c-3.3-7.9-3.3-16.7 0-24.6C17.3 208 48.6 156 95.4 112.6zM288 336c44.2 0 80-35.8 80-80s-35.8-80-80-80c-.7 0-1.3 0-2 0c1.3 5.1 2 10.5 2 16c0 35.3-28.7 64-64 64c-5.5 0-10.9-.7-16-2c0 .7 0 1.3 0 2c0 44.2 35.8 80 80 80zm0-208a128 128 0 1 1 0 256 128 128 0 1 1 0-256z" />
                      </svg>
                    </button>
                
                </div>
                <div className="flex justify-center items-center w-full h-full group ">
                  <img
                    src={item.image}
                    alt="Product"
                    className="w-[190px] h-[180px] object-contain "
                  />
                </div>
                {/*add to card  */}
                <button
                  className="text-center w-[100%] text-white bg-black p-[5px] absolute bottom-0 opacity-0 group-hover:!opacity-100 transition-opacity duration-300 cursor-pointer"
                  onClick={() => handleAddToCart(item)}
                >
                  Add to Cart
                </button>
                {/* add to card */}
              </div>

              {/* Card Body */}
              <div className=" h-[100px] bg-white mt-[16px]">
                <div className="title font-poppins font-[500] text-[16px] leading-[24px] truncate max-w-[240px]">
                  {item.title}
                </div>
                <div className="price text-[#DB4444] mt-2">
                  ${item.price}
                  <span className="text-[#aaaba7] ml-[10px] line-through">
                    $160
                  </span>
                </div>
                <Rating rate={item.rating.rate} count={item.rating.count} />
              </div>
              {/* Card Body */}
            </div>
          </div>
        ))}
      
    </div>
  );
});

export default Card;
